...
class URLService:
    ...
    def create_short_url(self, url_data: URLBase) -> URL:
        ...
        db_url = URL(target_url=url_data.target_url, key=key, secret_key=secret_key)
    ...
    def get_short_url_by_key(self, url_key: str) -> URL | None:
        ...
        return (
            self.db.query(URL).filter(URL.key == url_key, URL.is_active == True).first()
        )

    def get_short_url_by_secret_key(self, secret_key: str) -> URL | None:
        """Get active URL by secret key"""
        return self.db.query(URL).filter(URL.secret_key == secret_key).first()
    ...
    def delete_short_url(self, secret_key: str) -> URL | None:
        ...
        db_url = self.get_short_url_by_secret_key(secret_key)
        if db_url:
